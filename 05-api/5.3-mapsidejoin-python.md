...
```bash
    $   ~/dev/spark/bin/spark-submit  --master local[*]  python/mapsidejoin.py 
```
...
```bash
    $   ~/dev/spark/bin/spark-submit --master local[*]  python/mapsidejoin.py  2> logs
```
...
```bash
    ...
    $   ~/dev/spark/bin/spark-submit --master MASTER URL python/mapsidejoin.py  2>logs
```
...
```bash
    $  ~/dev/spark/bin/spark-submit --master MASTER_URL   python/mapsidejoin.py 2>  logs
```
...
```bash
    $   ~/dev/spark/bin/spark-submit --master local[*]  --driver-class-path logging/ python/mapsidejoin.py 
```
